...
void test_configuration_get_probe_name_fix_mounted_probe() {
  ...
  configuration.probeFixMountedProbe = true;
  TEST_ASSERT_EQUAL_STRING(configuration.getProbeName(), "FIX_MOUNTED_PROBE");
}

void test_configuration_get_probe_name_fix_mounted_probe_explicit() {
  Configuration configuration;
  TEST_ASSERT_EQUAL(configuration.getProbeName(CONF_PROBE_TYPE_FIX_MOUNTED_PROBE), "FIX_MOUNTED_PROBE");
}
